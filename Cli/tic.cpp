...
void HumanPlay::play(int sockfd)
{
  LOOP:
  ...
  char st[MAXLINE];
  if (fgets(st, MAXLINE, stdin) == NULL) {
    return;
  }
  ...
  int move = -1;
  if (cho == 1) {
    while (inputToken != 0 && move != 0)
    {
      system("clear");
      printf("欢迎来到TIC游戏室，请开始与AI对战:\n");
      board.printToScreen();
      printf("选择“0”退出当前游戏回到目录\n");
      char buf[MAXLINE];
      std::cout << "Your move: ";
...
void HumanPlay::p2pPlay(int sockfd)
{
  ...
  if (order == 1) {
    ...
    while (inputToken != 0 && move != 0)
    {
      ...
      if (board.isValid(inputToken))
      {
        ...
        if (!isWon && !isDraw)
        {
          ...
          board.move(move, PlayerEnum::X);
          bool isWon = checkWon(board, PlayerEnum::X, "You Lose");
          if (isWon)
          {
            system("clear");
            printf("欢迎来到TIC游戏室，请开始与对手对战:\n");
    ...
    while (inputToken != 0 && move !=0)
    {
      ...
      bool isWon = checkWon(board, PlayerEnum::O, "You Lose");
      bool isDraw = checkDraw(board);
